...
def aStar(start, goal, grid):
	...
	while openset:
		...
		for node in children(current,grid):
			...
			if node in openset:
				...
				node.H = manhattan(node, goal) #euclidean(node, goal) #manhattan(node, goal)
	...
	return []
def next_move(pacman, food, grid, pathname) -> (int, list):
...
def main():
    ...
    pacman_x, pacman_y = [20, 20]  #[ int(i) for i in input().strip().split() ]
    food_x, food_y = [200, 420]#[ int(i) for i in input().strip().split() ]
    ...
    print(grid)
    '''
    (pathNum, planPath) = next_move((pacman_x, pacman_y),(food_x, food_y), grid.tolist())
    
    outpathIMG(grid, pathNum, planPath)
        
    #print(grid[3][100:200])
